...
class MNISTDataModule(L.LightningDataModule):
    def __init__(self, data_dir: str = "../data/", batch_size: int = 200, num_workers: int = 1):
        super().__init__()
    ...
    def setup(self, stage: str):
        mnist_full_train_dataset = MNIST(root=self.data_dir, train=True, transform=ToTensor())
        train_len = int(len(mnist_full_train_dataset) * 0.8)  # 80% for training
        val_len = len(mnist_full_train_dataset) - train_len  # 20% for validation
        self.mnist_train, self.mnist_val = random_split(mnist_full_train_dataset, [train_len, val_len])
        self.mnist_test = MNIST(root=self.data_dir, train=False, transform=ToTensor())

    def train_dataloader(self):
        return self._dataloader(self.mnist_train, shuffle=True)

    def val_dataloader(self):
        return self._dataloader(self.mnist_val, shuffle=False)

    def test_dataloader(self):
        return self._dataloader(self.mnist_test, shuffle=False)
